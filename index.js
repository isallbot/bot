...
rdaftar = "WELCOME TO MEMBER BOT-ISALL😁"
limitt = 50
ban = []
userpremium = ["6289630267618@s.whatsapp.net"] //ubah nomer kalian
...
const getRegisteredRandomId = () => {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '*⏳ TUNGGU ANJM.*',
				success: '*JANGAN LUPA BILANG MAKASI🖕*',
				error: {
			...
			const ownerNumber = ["6289630267618@s.whatsapp.net"] // owner number ubah aja
     ...
     const bayarLimit = (sender, amount) => {
        ...
        const limitAdd = (sender) => {
            ...
            if (args.match(/(https:\/\/chat.whatsapp.com)/gi)) {
                ...
                if (!check) {
                    ...
                    reply('*[GROUP LINK DETECTOR!]*\nKamu mengirimkan link grup chat, TOLOL UDAH TAU GRUP TIDAK UUNTUK SHARE LINK GRUP.').then(() => {
		...
		case 'daftar':
					...
					if (umure.length >= 2, umure.length <= 1) return reply(`minimal umur 13 tahun keatas dan maximal 30 tahun`)
           ...
           if (!isUser) return reply(mess.only.userB)
            ...
            break  
				...
				case 'testime':
					setTimeout( () => {
					client.sendMessage(from, 'WAKTU UDAH HABIS TOLOL:v', text, {quoted: mek}) // ur cods
					}, 10000) // 1000 = 1s,
					setTimeout( () => {
				...
				case 'neonime':
                    ...
                    if (!isGroup) return reply(mess.only.group)
                    reply(mess.wait)
					teks = 'JANGAN LUPA BILANG MAKASIH\n'
					for (let i of data.result) {
						teks += `*Title* : ${i.judul}\n*link* : ${i.link}\n*rilis* : ${i.rilis}\n###############\n`
					}
					reply(teks.trim())
				...
				case 'unban':
					...
					reply(`Nomor wa.me/${bnnd} MAKANYA JANGAN NGELUNJAK KE OWNER!`)
					break
				case 'block':
				 client.updatePresence(from, Presence.composing) 
				 client.chatRead (from)
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					if (args.length < 2) return client.sendMessage(from, 'TEXS NYAA TOLOL?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
         ...
         case 'lirik':
				...
				case 'nsfwloli':
				    ...
				    if (!isUser) return reply(mess.only.userB)
				    if (isLimit(sender)) return reply(limitend(pushname2))
						if (!isNsfw) return reply(' *JANGAB LUPA BILANG MAKASIH ATAU DONASI* ')
...
// only grup fitur anime
              case 'anime':
                ...
                if (!isAnime) return reply(' *Harus Mengaktifkan Mode Anime* ')
					...
					client.sendMessage(from, pok, image, { quoted: mek , caption: 'MAKASIH GITU ANJING'})
					await limitAdd(sender) 
					break  
				case 'animekiss':
                if (isBanned) return reply(mess.only.benned)    
...
//akhir kreator
		    ...
		    case 'map':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					if (args.length < 2) return reply('URLNYA MANA TOD?')
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'MAKANYA JADIIN ADMIN GRUP DULU TOLOL :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'BILANG MAKASIH YA TOD'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
				...
				if (!isUser) return reply(mess.only.userB)
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('MANA KETIK ON ATAU OFF')
					if ((args[0]) === 'on') {
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					setTimeout( () => {
					client.sendMessage(from, '_60 Detik lagi_…', text) // ur cods
					}, 5000) // 1000 = 1s,
					setTimeout( () => {
